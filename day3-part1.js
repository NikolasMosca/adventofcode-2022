...
//Get all chars that are present in each strings 
const getDuplicates = (first, second) => Object.keys(
  [...first, ...second].reduce((duplicates, item) => {
...
const sumPriorities = inventory.reduce((priority, [first, second]) => (
  priority + getDuplicates(first, second).reduce((total, item) => total + getPriority(item), 0)
), 0)
