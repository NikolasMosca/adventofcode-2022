...
const checkRightOrder = (left, right) => {
  //If exactly one value is an integer, convert the integer to a list which contains that integer as its only value
  if(isArray(left) !== isArray(right)) {
    if(isArray(left) && !isArray(right)) right = [right]
    if(!isArray(left) && isArray(right)) left = [left]
  }
  console.log(`check right order => ${JSON.stringify(left)} - ${JSON.stringify(right)}`)

  //If there is an array then check with a recursion the value
  if(isArray(left)) {
    //If both values are lists, compare the first value of each list
    for(let i = 0; i < left.length; i++) {
      const leftElement = left[i]
      const rightElement = right[i]
      if(leftElement === null || leftElement === undefined) return true
      if(rightElement === null || rightElement === undefined) return false
      const result = checkRightOrder(leftElement, rightElement)
...
const orderedList = [...pairs, [[[2]], [[6]]]].reduce((data, [left, right]) => {
  if(data.length === 0) {
    const result = checkRightOrder(left, right)
    data = result ? [left, right] : [right, left]
  } else {
    let index = data.findIndex(item => checkRightOrder(item, left) === false)
    index = index >= 0 ? index : data.length
    data.splice(index, 0, left);

    index = data.findIndex(item => checkRightOrder(item, right) === false)
    index = index >= 0 ? index : data.length
    data.splice(index, 0, right);
  }
  return data
}, [])

const firstIndex = orderedList.findIndex(item => JSON.stringify(item) === '[[2]]') + 1
const secondIndex = orderedList.findIndex(item => JSON.stringify(item) === '[[6]]') + 1
