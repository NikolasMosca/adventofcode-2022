...
const { indexes } = pairs.reduce((data, [left, right], index) => {
  console.log(`============ ${index + 1}\n Compare ${JSON.stringify(left)} with ${JSON.stringify(right)} \n`)
  const result = checkRightOrder(left, right)
  if(result) {
    console.log('IS IN THE RIGHT ORDER')
    data.indexes.push(index + 1)
  } else if(result === false) {
    console.log('IS [NOT] IN THE RIGHT ORDER')
  }
  console.log(`\n\n`)
  return data
}, {
...
//Sum of indexes
console.log(indexes, indexes.reduce((total, item) => total + item, 0))
